...
export class MovieDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    // ActivatedRoute => This class will give us all the information of the current route/url
    // get the id from the URL 1, 2
    // then call our api, getMovieDetails method
    this.activeRoute.paramMap.subscribe   //need to subscribe because paramMap return observable
      (
        p => {
          this.id = Number(p.get('id'));    //need to convert to number
